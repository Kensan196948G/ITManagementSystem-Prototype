...
class MicrosoftGraphAPIClient:
    ...
    def __init__(
        ...
        if not all([self.tenant_id, self.client_id, self.client_secret]):
            raise ValueError(
                "tenant_id, client_id, client_secretは必須です。環境変数または引数で設定してください。"
            )
    ...
    def _get_token(self) -> None:
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"トークン取得失敗: {response.status_code} {response.text}"
            )
    ...
    def send_mail(self, user_id: str, message: dict) -> None:
        self.call_api(
            f"/users/{user_id}/sendMail", method="POST", json={"message": message}
        )
