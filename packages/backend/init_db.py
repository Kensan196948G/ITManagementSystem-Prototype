...
from packages.backend.database import Base, SessionLocal, engine, init_db
from packages.backend.models.change import Change, ChangePriority, ChangeStatus
...
from packages.backend.models.problem import Problem, ProblemPriority, ProblemStatus
from packages.backend.models.user import Permission, Role, User
...
def seed_reference_data():
    ...
    try:
        ...
        for status_data in statuses:
            status = (
                db.query(IncidentStatus).filter_by(name=status_data["name"]).first()
            )
            if not status:
        ...
        for priority_data in priorities:
            priority = (
                db.query(IncidentPriority).filter_by(name=priority_data["name"]).first()
            )
            if not priority:
        ...
        for priority_data in priorities:
            priority = (
                db.query(ProblemPriority).filter_by(name=priority_data["name"]).first()
            )
            if not priority:
        ...
        for priority_data in priorities:
            priority = (
                db.query(ChangePriority).filter_by(name=priority_data["name"]).first()
            )
            if not priority:
...
def seed_roles_and_permissions():
    ...
    try:
        # 基本ロール
        roles_data = [
            {"name": "admin", "description": "システム管理者", "is_system_role": True},
            {"name": "manager", "description": "マネージャー", "is_system_role": True},
            {"name": "operator", "description": "オペレーター", "is_system_role": True},
            {"name": "user", "description": "一般ユーザー", "is_system_role": True},
            {"name": "auditor", "description": "監査担当者", "is_system_role": True},
        ]
        ...
        permissions_data = [
            ...
            # インシデント管理
            {
                "code": "incident:read",
                "name": "インシデント閲覧",
                "description": "インシデントの閲覧",
            },
            {
                "code": "incident:write",
                "name": "インシデント編集",
                "description": "インシデントの作成・編集",
            },
            {
                "code": "incident:delete",
                "name": "インシデント削除",
                "description": "インシデントの削除",
            },
            # 問題管理
    ...
    try:
        ...
        if admin and john and jane:
            ...
            for incident_data in incidents_data:
                # 既存チェック（タイトルで判定）
                existing = (
                    db.query(Incident).filter_by(title=incident_data["title"]).first()
                )
                if not existing:
