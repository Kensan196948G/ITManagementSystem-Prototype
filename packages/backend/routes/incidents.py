...
from backend.models.incident import Incident, IncidentStatus, IncidentPriority, Comment, Attachment
...
# ファイルアップロード設定
UPLOAD_FOLDER = os.path.join(os.getcwd(), 'uploads', 'incidents') # プロジェクトルートからの相対パス
...
def get_incidents():
    ...
    assignee_filter = request.args.get('assignee') # assigneeのusernameでフィルタリングする場合
    ...
    # ページネーション
    paginated_incidents = query.order_by(Incident.created_at.desc()).paginate(page=page, per_page=limit, error_out=False)
...
def create_incident():
    ...
    if not title or not description or not priority_name:
        return jsonify({"error": "Missing required fields: title, description, priority"}), 400
    ...
    status_obj = IncidentStatus.query.filter_by(name="新規").first() # "新規" は実際のステータス名に合わせる
    if not status_obj:
         # 見つからない場合はエラー、またはデフォルトステータスを作成する処理
        return jsonify({"error": "Default status '新規' not found. Please ensure it exists in IncidentStatus table."}), 500
    ...
    if assignee_username:
        ...
        if assignee_user:
            ...
            return jsonify({"error": f"Assignee user '{assignee_username}' not found"}), 404
...
def get_incident_comments(incident_id):
    ...
    # TODO: 認証が必要
    incident = Incident.query.get_or_404(incident_id)
    comments_data = [comment.to_dict() for comment in incident.comments.order_by(Comment.created_at.asc()).all()]
...
def add_incident_comment(incident_id):
    ...
    if not user_id:
        return jsonify({"error": "User not found or not authenticated for commenting"}), 400
    ...
    if file:
        ...
        try:
            ...
            if not uploaded_by_id:
                 return jsonify({"error": "User not found or not authenticated for uploading"}), 400
            ...
            if os.path.exists(filepath):
                os.remove(filepath)
            return jsonify({"error": "Failed to upload attachment", "details": str(e)}), 500
