...
class RootCauseAnalysis(Base):
    ...
    identified_by_id = Column(
        Integer, ForeignKey("users.id"), nullable=True
    )  # Userモデルのusersテーブルを参照
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    problem = relationship("Problem", back_populates="root_cause_analyses")
    identified_by = relationship(
        "User", foreign_keys=[identified_by_id]
    )  # User とのリレーション (RCAを特定した人)
...
class Problem(Base):
    ...
    known_error_status = Column(
        String(100), nullable=True
    )  # 例: "確認済み", "解決策開発中", "恒久対策待ち" (既知のエラーDBとしてのステータス)
    ...
    category = relationship(
        "ProblemCategory", back_populates="problems", uselist=False
    )  # ProblemCategoryは一つなのでuselist=False

    reporter = relationship(
        "User", foreign_keys=[reporter_id], back_populates="reported_problems"
    )
    assignee = relationship(
        "User", foreign_keys=[assignee_id], back_populates="assigned_problems"
    )
    ...
    comments = relationship(
        "Comment", back_populates="problem", cascade="all, delete-orphan", lazy="select"
    )
