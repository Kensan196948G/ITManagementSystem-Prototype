...
class Incident(Base):  # Baseを継承
    ...
    def to_dict(self, include_details=False):
        data = {
            ...
            "assignee": (
                self.assignee.username if self.assignee else None
            ),  # assigneeのusernameを表示
            "reporter_id": self.reporter_id,
            "reporter": (
                self.reporter.username if self.reporter else None
            ),  # reporterのusernameを表示
            "created_at": self.created_at.isoformat(),
            "updated_at": self.updated_at.isoformat(),
            "comments_count": (
                len(self.comments) if self.comments else 0
            ),  # lazy='dynamic'ではないのでlen()を使用
            "attachments_count": (
                len(self.attachments) if self.attachments else 0
            ),  # lazy='dynamic'ではないのでlen()を使用
        }
        if include_details:
            data["comments"] = [
