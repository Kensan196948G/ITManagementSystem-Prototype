...
class ExecutionManager:
    ...
    def execute_code(self, code_to_execute, timeout=10):
        ...
        if not isinstance(code_to_execute, str):
            ...
            return {"stdout": "", "stderr": "Invalid input: code_to_execute must be a string."}
        ...
        try:
            ...
            execution_result = {"stdout": "", "stderr": f"Execution timed out after {timeout} seconds."}
            ...
            if len(e.stderr) > max_output_size:
                 stderr += "\nError output truncated due to size limit."
            execution_result = {"stdout": stdout, "stderr": stderr + f"\nCommand failed with exit code {e.returncode}"}
        except Exception as e:
            # 修正ポイント: 実行結果の安全な取得 - その他のエラーメッセージ
            execution_result = {"stdout": "", "stderr": f"An error occurred during code execution: {e}"}
    ...
    def execute_command(self, command, timeout=10):
        ...
        if any(forbidden in command for forbidden in forbidden_commands):
             return {"stdout": "", "stderr": "Potential security risk detected in command."}
        ...
        try:
            ...
            # 修正ポイント: 実行結果の安全な取得 - タイムアウト時のエラーメッセージ
            command_result = {"stdout": "", "stderr": f"Command timed out after {timeout} seconds."}
        except subprocess.CalledProcessError as e:
            # 修正ポイント: 実行結果の安全な取得 - プロセスエラー時のエラーメッセージと出力制限
            max_output_size = 1024 * 1024 # 例: 1MB
            stdout = e.stdout[:max_output_size]
            ...
            if len(e.stderr) > max_output_size:
                 stderr += "\nError output truncated due to size limit."
            command_result = {"stdout": stdout, "stderr": stderr + f"\nCommand failed with exit code {e.returncode}"}
        except Exception as e:
            # 修正ポイント: 実行結果の安全な取得 - その他のエラーメッセージ
            command_result = {"stdout": "", "stderr": f"An error occurred during command execution: {e}"}
