...
class ErrorAnalyzer:
    def analyze_errors(self, errors):
        ...
        for error in errors:
            ...
            if "FileNotFoundError" in error.get("message", "") or "No such file or directory" in error.get("message", ""):
                error_type = "File Not Found"
                estimated_cause = "Missing file"
                severity = "high"
            elif "database" in error.get("message", "") or "SQLAlchemy" in error.get("message", ""):
                error_type = "Database Error"
                estimated_cause = "Database connection or query issue"
                severity = "high"
            elif "Authentication" in error.get("message", "") or "JWT" in error.get("message", ""):
