...
from backend.database import Base
# User, Incidentモデルは直接インポートせず、relationshipで文字列で参照
# from .user import User
# from .incident import Incident, Comment, Attachment
...
class Problem(Base):
    ...
    reported_by_id = Column(Integer, ForeignKey('users.id'), nullable=False) # Userモデルのusersテーブルを参照
    assigned_to_id = Column(Integer, ForeignKey('users.id'), nullable=True) # Userモデルのusersテーブルを参照
    ...
    reporter = relationship('User', foreign_keys=[reported_by_id], back_populates='reported_problems')
    assignee = relationship('User', foreign_keys=[assigned_to_id], back_populates='assigned_problems')
