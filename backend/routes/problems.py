...
async def create_problem(
    ...
    current_user: models.User = Depends(get_current_active_user) # Userスキーマはdependenciesのもの
):
    """
    新しい問題を作成します。
    - **title**: 問題のタイトル (必須)
    ...
    if not db.query(models.ProblemStatus).filter(models.ProblemStatus.id == problem_in.status_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"ProblemStatus with id {problem_in.status_id} not found.")
    if not db.query(models.ProblemPriority).filter(models.ProblemPriority.id == problem_in.priority_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"ProblemPriority with id {problem_in.priority_id} not found.")
    if problem_in.category_id and not db.query(models.ProblemCategory).filter(models.ProblemCategory.id == problem_in.category_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"ProblemCategory with id {problem_in.category_id} not found.")
    if problem_in.assigned_to_id and not db.query(models.User).filter(models.User.id == problem_in.assigned_to_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User (assignee) with id {problem_in.assigned_to_id} not found.")
    ...
    if problem_in.linked_incident_ids:
        for incident_id in problem_in.linked_incident_ids:
            ...
            if incident:
                link = models.ProblemIncidentLink(problem_id=db_problem.id, incident_id=incident_id)
                db.add(link)
            else:
                # 存在しないインシデントIDが指定された場合の警告ログ等をここに追加可能
                print(f"Warning: Incident with id {incident_id} not found when linking to problem {db_problem.id}")
        db.commit()
...
async def update_problem(
    ...
    update_data = problem_in.model_dump(exclude_unset=True, exclude={"linked_incident_ids"})

    # 存在チェック: status_id, priority_id, category_id, assigned_to_id
    if "status_id" in update_data and not db.query(models.ProblemStatus).filter(models.ProblemStatus.id == update_data["status_id"]).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"ProblemStatus with id {update_data['status_id']} not found.")
    if "priority_id" in update_data and not db.query(models.ProblemPriority).filter(models.ProblemPriority.id == update_data["priority_id"]).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"ProblemPriority with id {update_data['priority_id']} not found.")
    if "category_id" in update_data and update_data["category_id"] is not None and not db.query(models.ProblemCategory).filter(models.ProblemCategory.id == update_data["category_id"]).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"ProblemCategory with id {update_data['category_id']} not found.")
    if "assigned_to_id" in update_data and update_data["assigned_to_id"] is not None and not db.query(models.User).filter(models.User.id == update_data["assigned_to_id"]).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User (assignee) with id {update_data['assigned_to_id']} not found.")
    ...
    if problem_in.linked_incident_ids is not None:
        # 既存のリンクを一旦全て削除
        db.query(models.ProblemIncidentLink).filter(models.ProblemIncidentLink.problem_id == problem_id).delete()
        # 新しいリンクを追加
        for incident_id in problem_in.linked_incident_ids:
            incident = db.query(models.Incident).filter(models.Incident.id == incident_id).first()
            if incident:
                link = models.ProblemIncidentLink(problem_id=problem_id, incident_id=incident_id)
                db.add(link)
            else:
                print(f"Warning: Incident with id {incident_id} not found when updating links for problem {problem_id}")
...
@router.post("/{problem_id}/rca", response_model=schemas.RootCauseAnalysis, status_code=status.HTTP_201_CREATED)
async def create_root_cause_analysis(
    ...
    """
    db_problem = get_problem_or_404(problem_id, db)
    if rca_in.identified_by_id and not db.query(models.User).filter(models.User.id == rca_in.identified_by_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User (identifier) with id {rca_in.identified_by_id} not found.")
...
async def read_root_cause_analyses(
    problem_id: int,
    db: Session = Depends(get_db)
):
    """
...
@router.post("/{problem_id}/workarounds", response_model=schemas.Workaround, status_code=status.HTTP_201_CREATED)
async def create_workaround(
    ...
    """
    db_problem = get_problem_or_404(problem_id, db)
    if workaround_in.implemented_by_id and not db.query(models.User).filter(models.User.id == workaround_in.implemented_by_id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User (implementer) with id {workaround_in.implemented_by_id} not found.")

    db_workaround = models.Workaround(
        **workaround_in.model_dump(),
        problem_id=db_problem.id
    )
...
@router.put("/{problem_id}/workarounds/{workaround_id}", response_model=schemas.Workaround)
async def update_workaround(
    ...
    """
    db_problem = get_problem_or_404(problem_id, db)
    db_workaround = db.query(models.Workaround).filter(
        models.Workaround.id == workaround_id,
        models.Workaround.problem_id == problem_id
    ).first()
    if not db_workaround:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Workaround with id {workaround_id} for problem {problem_id} not found")

    update_data = workaround_in.model_dump(exclude_unset=True)
    if "implemented_by_id" in update_data and update_data["implemented_by_id"] is not None and not db.query(models.User).filter(models.User.id == update_data["implemented_by_id"]).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"User (implementer) with id {update_data['implemented_by_id']} not found.")
...
@router.delete("/{problem_id}/workarounds/{workaround_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.post("/{problem_id}/comments", response_model=schemas.Comment, status_code=status.HTTP_201_CREATED)
...
@router.post("/{problem_id}/attachments", response_model=schemas.Attachment, status_code=status.HTTP_201_CREATED)
...
async def get_problem_attachments(
    ...
    attachments = db.query(models.Attachment).filter(models.Attachment.problem_id == problem_id).all()
    return attachments

# --- マスタデータ取得API ---

@router.get("/statuses/", response_model=List[schemas.ProblemStatus]) # 末尾のスラッシュで /statuses と区別
