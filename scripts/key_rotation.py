...
class KeyRotationManager:
    ...
    def _encrypt_backup_file(self, backup_file: Path) -> Path:
        ...
        try:
            # マスターパスワードから暗号化キーを派生
            master_password = os.environ.get('BACKUP_MASTER_PASSWORD', 'default_backup_key').encode()
    ...
    def rotate_key(self, key_type: str) -> Dict[str, str]:
        ...
        if len(key_history['keys'][key_type]) > config['backup_versions']:
            ...
            key_history['keys'][key_type] = key_history['keys'][key_type][-config['backup_versions']:]
        ...
        rotation_entry = {
            'key_type': key_type,
            'rotation_date': datetime.now().isoformat(),
            'old_key_hash': hashlib.sha256(current_key.encode()).hexdigest()[:16] if current_key else None,
        ...
        for key_type, config in self.rotation_config.items():
            ...
            if last_rotation is None:
                ...
                if datetime.now() >= next_rotation:
                    ...
                    priority = 'critical' if days_overdue > 30 else 'high' if days_overdue > 7 else 'medium'
    ...
    def generate_rotation_report(self) -> Dict:
        ...
        report = {
            ...
            'rotation_history': key_history.get('rotation_history', [])[-10:],  # 最新10件
            'recommendations': []
        }
...
def main():
    parser = argparse.ArgumentParser(description='ITサービス管理システム キーローテーション管理')
    parser.add_argument('--rotate', choices=['jwt', 'session', 'csrf', 'encryption', 'all'], 
                       help='ローテーションするキータイプ')
    parser.add_argument('--schedule', action='store_true', help='ローテーションスケジュールをチェック')
    parser.add_argument('--verify', action='store_true', help='キーの整合性を検証')
    parser.add_argument('--report', action='store_true', help='ローテーションレポートを表示')
    parser.add_argument('--update-env', action='store_true', help='環境変数ファイルを自動更新')
    ...
    try:
        ...
        if args.verify:
            ...
            for key_type, is_valid in integrity.items():
                ...
                print(f"  {status_icon} {key_type}: {'正常' if is_valid else '問題あり'}")
        ...
        if args.report:
            ...
            print(f"ローテーション必要: {report['key_status']['keys_needing_rotation']}")
