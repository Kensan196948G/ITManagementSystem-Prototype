...
const IncidentManagementPage = () => {
    ...
    const handleCreateModalOpen = () => {
        setNewIncidentData({ title: '', description: '', priority: '', assigned_to_id: null });
                        ...
                        <table className="min-w-full w-full table-auto">
                            <thead className="bg-gray-100">
                                <tr className="text-gray-600 uppercase text-sm leading-normal">
                                    <th className="py-3 px-6 text-left">ID</th><th className="py-3 px-6 text-left">タイトル</th><th className="py-3 px-6 text-center">ステータス</th><th className="py-3 px-6 text-center">優先度</th><th className="py-3 px-6 text-left">担当者</th><th className="py-3 px-6 text-left">作成日時</th><th className="py-3 px-6 text-center">アクション</th>
                                </tr>
                            </thead>
                ...
                {isDetailModalOpen && selectedIncident && (
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                        <div className="relative top-10 mx-auto p-6 border w-full max-w-3xl shadow-lg rounded-md bg-white">
                            <div className="flex justify-between items-center pb-3 border-b mb-4"><h3 className="text-2xl font-semibold text-gray-800">インシデント詳細: ID {selectedIncident.id}</h3><button onClick={handleDetailModalClose} className="text-gray-400 hover:text-gray-600 text-2xl">&times;</button></div>
                            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
                                    <div><p className="text-sm font-medium text-gray-500">タイトル</p><p className="text-lg text-gray-800">{selectedIncident.title}</p></div>
                                    <div><p className="text-sm font-medium text-gray-500">ステータス</p><Badge variant={getStatusBadgeVariant(selectedIncident.status_name || selectedIncident.status)}>{selectedIncident.status_name || selectedIncident.status}</Badge></div>
                                    <div><p className="text-sm font-medium text-gray-500">優先度</p><Badge variant={getPriorityBadgeVariant(selectedIncident.priority_name || selectedIncident.priority)}>{selectedIncident.priority_name || selectedIncident.priority}</Badge></div>
                                    <div><p className="text-sm font-medium text-gray-500">担当者</p><p className="text-gray-800">{selectedIncident.assignee_name || selectedIncident.assigned_to || '未割り当て'}</p></div>
                                    <div><p className="text-sm font-medium text-gray-500">作成日時</p><p className="text-gray-800">{selectedIncident.created_at ? new Date(selectedIncident.created_at).toLocaleString('ja-JP') : 'N/A'}</p></div>
                                    <div><p className="text-sm font-medium text-gray-500">最終更新日時</p><p className="text-gray-800">{selectedIncident.updated_at ? new Date(selectedIncident.updated_at).toLocaleString('ja-JP') : 'N/A'}</p></div>
                                </div>
                                <div className="pt-2"><p className="text-sm font-medium text-gray-500">説明</p><p className="text-gray-800 whitespace-pre-wrap">{selectedIncident.description || '説明はありません。'}</p></div>
                ...
                {isDeleteConfirmModalOpen && (
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
                        <div className="relative top-1/3 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
                            ...
                            <p className="text-gray-600 mb-6">インシデント ID: {incidentIdToDelete} を本当に削除しますか？この操作は元に戻せません。</p>
